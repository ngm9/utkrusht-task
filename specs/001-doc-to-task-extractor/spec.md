...
The customer brief contains a URL pointing to an external code-hosting service that holds the starter code the candidate must build upon. The extractor must fetch the source files from that URL **without circumventing any bot-protection mechanism** the platform has deployed, and embed the raw source code byte-for-byte in the per-task markdown as "starter code blocks", while ensuring the original URL is **not** present anywhere in the markdown. The LLM agent sees a single tool (`fetch_external_code(url)`) regardless of which platform the URL points to; platform-specific fetching logic is internal to the tool. When the URL points to a bot-protected platform (e.g., CodePen behind Cloudflare), the tool returns `status="bot_protected"` and the agent emits a clear operator-facing note in the markdown asking for manual paste — **the tool MUST NOT attempt to bypass Cloudflare, CAPTCHA, or similar protection** (per constitution Principle V).

**Why this priority**: Starter-code-from-external-URL is the second-most-common resource type after images — customer briefs frequently link out to CodePen pens, public Gists, CodeSandbox projects, or similar to convey the starter scaffold a candidate should build on. For unprotected platforms (GitHub Gist), the extractor saves the operator a manual copy-paste step. For bot-protected platforms (CodePen), the extractor still helps by clearly flagging the protection and giving the operator a deterministic next step — which is better than today's manual workflow where the operator discovers the problem only after copy-paste failure.

**Independent Test**: Run the extractor on a brief that references a public GitHub Gist URL. Verify the markdown contains one fenced code block per Gist file with the contents byte-for-byte from the Gist, and the original Gist URL appears nowhere in the markdown. Separately, run on a brief that references a CodePen URL and verify the markdown contains a `**Note:**` paragraph asking the operator to paste the source manually (not a partial / corrupted scrape).

**Acceptance Scenarios**:

1. **Given** a brief referencing a public GitHub Gist, **When** the extractor runs, **Then** the markdown contains each Gist file as a fenced code block with its filename; the Gist URL appears in no output file.
2. **Given** a brief referencing a public CodePen pen (which is gated by Cloudflare's bot protection), **When** the extractor runs, **Then** `fetch_external_code` returns `status="bot_protected"` and the markdown contains an inline `**Note:**` paragraph asking the operator to open the link, copy the source file-by-file into the document, and re-run the parser; **no Cloudflare bypass is attempted**, no partial content is embedded, and the CodePen URL never appears in the markdown.
3. **Given** a brief referencing an external URL whose platform is not yet supported by the fetcher (e.g., CodeSandbox, JSFiddle, Pastebin, Replit in v1), **When** the extractor runs, **Then** the per-task markdown includes an inline note in the relevant section explaining that the referenced platform is not yet supported and the operator should paste the source manually; no partial or corrupted starter code is embedded.
...
5. **Given** the same external URL is referenced again in a re-run, **When** the extractor runs, **Then** the second run reads the cached content from `tmp/parser_cache/<sha256(url)>.json` and does not re-fetch over the network (within-session idempotency).
...
- **FR-007**: System MUST expose **a single tool** `fetch_external_code(url)` to the LLM for fetching starter code from an external URL. The tool's surface to the LLM is platform-agnostic — the LLM gives a URL, the tool returns a status, a dict of `{filename: source_content}` (when status is `"ok"`), and a `platform_detected` field. Internal dispatch by URL pattern selects the right fetcher; the LLM never sees per-platform branches.
  - **v1 scope — fully supported**: GitHub Gist (plain HTTP against the public Gists API; no auth required for public gists).
  - **v1 scope — bot-protected**: CodePen (sits behind Cloudflare's JS challenge). The tool MUST NOT bypass the protection (constitution Principle V). For these URLs the tool returns `status="bot_protected"` with a canonical operator-facing message asking the user to paste the source manually and re-run.
  - **v1 not-yet-supported** (CodeSandbox, JSFiddle, Pastebin, Replit, GitLab Snippets): the tool MUST detect these URLs and return `status="platform_not_supported"` that the LLM agent surfaces as an inline `**Note:**` in the affected task's markdown. Adding a new platform later is a per-platform module addition; no spec change required.
...
- **FR-012**: System MUST be invocable as `python -m task_input_parser <brief-path>` (sub-package CLI convention per constitution Principle II).
...
- **SC-003**: For a brief referencing a public GitHub Gist URL, the starter code embedded in the per-task markdown is **byte-for-byte identical** to the Gist's source files. For a brief referencing a CodePen URL, the markdown contains a clear `**Note:**` directing the operator to manually paste the source — no bypass is attempted and no partial content is emitted.
...
- **SC-009**: The end-to-end run time for the Nerdium brief (2 tasks, 1 image upload, 1 CodePen URL flagged as bot-protected) is **under 60 seconds** on a typical developer workstation. (No Chrome window is launched; no Cloudflare challenge to wait through.)
