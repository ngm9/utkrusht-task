#!/usr/bin/env bash
# Sandbox boot script for utkrusht-rust: bring up dockerd + browser terminal
# + browser IDE + DB GUI in the background, then keep the VM alive so the E2B
# SDK can connect. Same shape as templates/python-base/start.sh — these
# services are runtime-agnostic.
set -e
