...
> **Updated:** 2026-05-27
> **Sister doc:** [Task Classifier](./classifier.md) — the design owner. This registry's rows ARE the capability sheets the classifier reads to pick `template_id` + `persona`. The schema evolution (`personas[]`, `manifest_hash`, `registry_version`, `template_id` as PK) and migration path live in the classifier doc; this doc covers the operational details: what's baked into each image, how E2B templates are built, fat-vs-composition trade-offs, and how `run.sh` relates to the registry's build/test commands.
...
1. **Wire `sandbox_eval` to read commands from `template_registry`** (~15 LOC). Turns the registry from "stored but unused" into "actively consumed." Validates the pattern before the schema evolution (Steps 1–2 in the [classifier doc's migration path](./classifier.md#migration-path)) lands.
2. **Build the second template (`utkrusht-node-base`) with the new schema from day one** — `personas: text[]` populated, `manifest_hash` generated by the build pipeline. Retrofitting these onto a growing template set is harder than starting with them. This step exercises both the multi-runtime path and the capability-sheet shape end-to-end.
