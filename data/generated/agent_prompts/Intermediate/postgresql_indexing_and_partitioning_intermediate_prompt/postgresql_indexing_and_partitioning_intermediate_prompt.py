PROMPT_POSTGRESQL_INDEXING_PARTITIONING_INTERMEDIATE_CONTEXT = """
...
PROMPT_POSTGRESQL_INDEXING_PARTITIONING_INTERMEDIATE_INPUT_AND_ASK = """
...
- Ensure the candidate can realistically complete the task in the allocated time ({minutes_range}).
...
2. What will the task look like? (Describe the type of PostgreSQL indexing and partitioning optimization or schema design required, the expected deliverables, and how it aligns with the given proficiency level)

Please provide a brief summary of your understanding before proceeding with the full task generation.
"""

PROMPT_POSTGRESQL_INDEXING_PARTITIONING_INTERMEDIATE_INSTRUCTIONS = """
## GOAL
As a database architect experienced in PostgreSQL, you are given a list of real-world scenarios and proficiency levels for PostgreSQL.
Your job is to generate a task, with the given specifications, so that a candidate is presented with a functional PostgreSQL database with initial schema and data but with logical and performance issues that require intermediate-level indexing and partitioning skills.
...
- Intentionally complex, inefficient queries, missing or suboptimal indexes, or poor partitioning strategies
- Performance bottlenecks that demand intermediate-level problem-solving (3–5 years experience)
- Real-world business scenarios requiring database optimization, especially in indexing and partitioning

The candidate's responsibility is to analyze the database, identify performance issues, and implement PostgreSQL indexing and partitioning optimizations directly using SQL commands, psql, or any database client tool of their choice.

## INSTRUCTIONS

### Nature of the Task 
- Task name MUST be within 50 words and clearly describe the intermediate-level indexing and partitioning optimization scenario.
- Task must provide a working database with existing schema, data, and intentionally suboptimal design requiring intermediate-level optimization skills in indexing and partitioning.
...
- **PROVIDE COMPLEX PROBLEMATIC DATABASE DESIGN**: Include init_database.sql with multiple inefficient table structures, missing or suboptimal indexes (including multi-column, partial, or expression indexes), poor or missing partitioning, and data distributions that expose performance issues.
- The question should be a real-world business scenario requiring intermediate-level database performance optimization involving multiple tables, complex queries, and advanced indexing and partitioning strategies.
- The complexity of the optimization task and specific improvements expected from the candidate must align with intermediate proficiency level (3–5 years experience) requiring:
  - Advanced indexing strategies (composite, partial, covering/INCLUDE, GIN, GiST, BRIN)
  - Query plan analysis and interpretation using EXPLAIN/ANALYZE
  - Partitioning strategies (range, list, hash), partition pruning, and management (ATTACH, DETACH, DROP)
  - Catalog views (pg_stat_all_indexes, pg_index, pg_partitioned_table) and extensions (pg_stat_statements, auto_explain, pg_partman)
  - Maintenance operations (REINDEX, CLUSTER, pg_repack), VACUUM/autovacuum coordination
  - Schema changes and transactional safety
  - Understanding of index/partition impact on backup, recovery, and edge cases (skew, bloat)
...
- Ensure that all questions and scenarios adhere to the latest PostgreSQL best practices and versions for intermediate-level indexing and partitioning.

## Database Generation Instructions:
Based on the real-world scenarios provided, create a PostgreSQL indexing and partitioning optimization task that:
...
- Tests practical intermediate-level PostgreSQL indexing and partitioning optimization and performance tuning skills that require deep understanding of database internals, query execution plans, and advanced optimization principles.
- Time constraints: Each task should be finished within {minutes_range} minutes.
- At every time pick a different real-world scenario from the list provided to ensure variety in task generation.
- **CRITICAL**: The PostgreSQL database should be COMPLETE and FULLY POPULATED with realistic data, but with intentionally complex inefficient schema, missing or suboptimal indexes and partitioning, and poor query performance requiring intermediate-level optimization.
...
- Include sample queries in the documentation that demonstrate the performance problems (slow execution times and poor query plans).
...
- Include intentional performance problems such as:
  ...
  - Missing composite or partial indexes for multi-column queries
  - Inefficient data types
  - Poor normalization or over-normalization
  - Missing foreign key indexes
  - Tables without primary keys or using inefficient primary keys
  - No partitioning on large tables that would benefit from it, or poorly designed partitions
...
## README.md INSTRUCTIONS:
 - The README.md contains the following sections:
   ...
   - Objectives
   - How to Verify
- The README.md file content MUST be fully populated with meaningful, specific content relevant to intermediate-level indexing and partitioning optimization challenges
...
- Content must be directly relevant to the specific complex PostgreSQL indexing and partitioning optimization scenario being generated
...
### Guidance
  - **Advanced database context** and architectural guidance points for PostgreSQL indexing and partitioning optimization
  - **Technical architecture notes** covering PostgreSQL indexing strategies, query optimization patterns, partitioning considerations, and performance tuning
...
### Objectives
  - Clear, measurable goals for the candidate focusing on advanced database performance optimization requiring intermediate-level skills in indexing and partitioning
  ...
  - Should focus on complex performance improvement objectives with measurable outcomes (significant query execution time improvements, specific query optimization, index and partition implementation)
...
{{
   ...
   "question": "A short description of the intermediate-level indexing and partitioning optimization task scenario including the specific complex performance problems in the existing database that need to be identified and resolved using advanced techniques — what complex performance bottlenecks exist involving multiple tables and advanced queries, and what intermediate-level indexing and partitioning optimizations are needed?",
   "code_files": {{
      "README.md": "Candidate-facing README with Task Overview, Database Schema Overview, Database Access, Guidance, Objectives, How to Verify, and Tools for intermediate-level indexing and partitioning optimization",
      ...
      "init_database.sql": "Complete database initialization file containing both complex schema creation with multiple indexing and partitioning performance issues AND comprehensive sample data insertion - ensures proper execution order in Docker",
   ...
   "outcomes": "Expected results after completion in 2–3 lines focusing on measurable complex performance improvements and optimized database operations requiring intermediate-level indexing and partitioning skills. Use simple english.",
   "short_overview": "REQUIRED. A bullet-point list of 3–5 short, candidate-facing bullets in simple language describing: (1) the business/technical problem with the existing PostgreSQL workload (high-churn tables, bloated indexes, missing partition pruning, unstable query plans, etc.), (2) the specific indexing and partitioning optimisation goal the candidate must achieve, and (3) the expected measurable outcome (e.g., stable sub-100ms p95 on the hot query, partition pruning visible in EXPLAIN, removal of redundant indexes, reduced bloat). Each bullet must start with '- ' and be one sentence. Do NOT leak the solution — describe the symptoms and the target end-state, not which specific indexes/partitions to create. MUST be a non-empty list of strings; an empty value will fail downstream validation.",
   "pre_requisites": "Bullet-point list of tools, knowledge, and environment required to complete the intermediate-level indexing and partitioning optimization task. Mention things like Docker, Docker Compose, PostgreSQL client tools (pgAdmin/DBeaver/psql), basic SQL knowledge, understanding of EXPLAIN ANALYZE, intermediate PostgreSQL concepts (indexes, partitioning, query optimization, normalization), etc.",
   "answer": "High-level solution approach focusing on database optimization strategies and intermediate-level indexing and partitioning performance tuning techniques for the given complex performance issues. Include specific optimization techniques like: which indexes to create, query rewrites needed, partitioning strategies, schema changes required, configuration tuning, etc.",
   "hints": "A single line hint on what a good intermediate-level approach to analyze and optimize the database performance could include. These hints must NOT give away the specific optimizations needed, but gently nudge the candidate toward database performance analysis practices suitable for intermediate-level skills (e.g., 'Start by analyzing query execution plans to identify sequential scans, missing indexes, and partition pruning opportunities').",
   "definitions": {{
...
"""
PROMPT_REGISTRY = {
    "PostgreSQL - Indexing and Partitioning (INTERMEDIATE)": [
        PROMPT_POSTGRESQL_INDEXING_PARTITIONING_INTERMEDIATE_CONTEXT,
        PROMPT_POSTGRESQL_INDEXING_PARTITIONING_INTERMEDIATE_INPUT_AND_ASK,
        PROMPT_POSTGRESQL_INDEXING_PARTITIONING_INTERMEDIATE_INSTRUCTIONS,
    ]
