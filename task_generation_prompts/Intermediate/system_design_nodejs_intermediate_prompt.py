PROMPT_SYSTEM_DESIGN_NODEJS_CONTEXT_INTERMEDIATE = """
...
PROMPT_SYSTEM_DESIGN_NODEJS_INPUT_AND_ASK_INTERMEDIATE = """
Now that you understand the company context and role requirements, let me provide you with the specific inputs for generating a Node.js System Design assessment task.
...
- **Service Architecture**: Designing multi-service systems — service boundaries, inter-service communication patterns, API design, modular monolith vs microservices trade-offs
- **Event Loop & Concurrency**: Single-threaded event loop implications, async/await and Promise patterns, blocking vs non-blocking I/O, CPU-bound work offloading (worker threads, child processes, external workers), back-pressure handling
- **Data Access Patterns**: Database access strategies, connection management/pooling, caching layers, read replicas, N+1 prevention, transactional consistency, migration strategies
- **Background Task Processing**: Job queue design, scheduled jobs, long-running task management, retry/dead-letter strategies, worker pool sizing
- **Event-Driven Architecture**: Message broker design, event schemas, delivery guarantees (at-least-once, exactly-once), consumer strategies, dead letter handling, idempotent consumers
- **API Design & Gateway Patterns**: RESTful and/or GraphQL API design, request/response model design, validation, API versioning, rate limiting, middleware composition, authentication/authorization
- **Caching & Performance**: Caching strategies, cache invalidation patterns, response caching, query caching, payload serialization efficiency
...
- **Observability Design**: Structured logging, distributed tracing, metrics, health checks, alerting, request correlation IDs
- **Deployment & Scaling**: Process model (single process vs cluster vs container replicas), horizontal scaling, stateless service design, container sizing, graceful shutdown, deployment strategies, zero-downtime rollouts
...
{{
   ...
   "code_files": {{
      ...
      ".gitignore": "Standard Node.js .gitignore (node_modules/, dist/, .env, logs, coverage/, etc.)",
      ...
      "// Build/config files": "package.json with real dependencies and scripts, tsconfig.json (if TypeScript), .env.example, docker-compose.yml if applicable, eslint/prettier config if applicable",
      "// Source code files": "HTTP server entrypoint (e.g., src/server.js or src/index.ts), route definitions (e.g., src/routes/...), controllers/handlers (e.g., src/controllers/...), service modules (e.g., src/services/...), data models / ORM models (e.g., src/models/...), repository/DAO modules (e.g., src/repositories/...), middleware (e.g., src/middleware/...), config module (e.g., src/config/...), worker / job processor modules (e.g., src/workers/...), message consumer/producer modules if applicable",
      "// SQL/schema files": "Database schema or migration files if applicable (e.g., migrations/*.js for Knex/Sequelize, prisma/schema.prisma, or schema.sql)"
   }},
   ...
   "pre_requisites": "Bullet-point list of knowledge areas needed: understanding of Node.js runtime model and HTTP server frameworks, experience with databases and data access patterns in Node, familiarity with message brokers and event-driven systems, knowledge of the event loop and async patterns, understanding of system scaling, the cluster/worker model, and resource management, ability to create architecture diagrams in text or mermaid format.",
   "answer": "Thorough reference design approach — the evaluator's answer key. Must include: recommended technology/framework choices with rationale, specific data access strategy, concurrency / event-loop approach, inter-service communication design, event schema examples, failure handling patterns, and deployment considerations. This should be 4-6 paragraphs covering each DESIGN.md section. Note: this is for the EVALUATOR only — candidates never see this.",
   "hints": "A single line nudge toward the right design direction. Example: 'Consider how a job queue and worker pool could offload CPU-bound work and protect the request-handling event loop from saturation.'",
   "definitions": {{
     "term1": "definition1",
     "term2": "definition2"
   }}
...
- Forgetting to close parentheses in database nodes: `DB[(MongoDB]` instead of `DB[(MongoDB)]`
...
The repo must contain the actual Node.js project folder structure with real code files showing the current system. This is NOT pseudo code and NOT a full line-by-line production implementation. It is structural/high-level code that shows:

**What the code files SHOULD contain:**
- Real module/file structure with proper folder organization (e.g., `src/routes/orders.js`, `src/controllers/order_controller.js`, `src/services/order_service.js`, `src/models/order.js`)
- Function/method signatures with brief high-level logic (what the function does, not every line)
- Real route definitions, controller handlers, service modules, model schemas, middleware, and config
- Configuration files (`package.json` with real dependencies and scripts, `tsconfig.json` if TypeScript, config/settings module, `.env.example`)
- Middleware modules showing auth, validation, error-handling middleware composition
- Service-layer modules showing business logic flow at a high level
- Repository/DAO modules or model files showing data access patterns
- Worker / job processor modules if applicable (e.g., `src/workers/email_worker.js`)
- Message consumer/producer modules if applicable
- `docker-compose.yml` if the system uses containers
- Migration files (e.g., `migrations/0001_initial.js` for Knex/Sequelize, or `prisma/schema.prisma`) or `schema.sql` if applicable
...
**Style guideline:**
- Each code file should feel like reading a real project's source — proper `require`/`import` statements, exports, async/await usage, real method signatures, real schemas
- Functions should have real signatures and show the high-level flow (e.g., "fetch from DB, transform, publish to queue") with enough code to understand what happens, but not every utility method or edge case
- Config files should show real dependencies (`package.json`), database connection setup, broker settings
...
- Follow standard Node.js project conventions (e.g., `src/...` layout with `routes/`, `controllers/`, `services/`, `models/`, `repositories/`, `middleware/`, `workers/`, `config/`)
- Include build files (`package.json`, optionally `tsconfig.json`), config files, source files, and schema/migration files as appropriate
...
2. **This is a DESIGN task with real code context** — The repo contains actual Node.js project code files (source modules, config, build files) showing the current system. The candidate reads these to understand the current state, then writes their design in DESIGN.md.
...
8. **answer field** is the evaluator's answer key — make it thorough with specific technology choices, data model details, concurrency / event-loop strategy, and trade-off rationale. This is for evaluators only, never shown to candidates.
9. **definitions** must include 4-6 relevant architecture/design terms (e.g., circuit breaker, saga pattern, connection pool, event sourcing, back-pressure, idempotency key, event loop, cluster mode)
...
11. **Code files must be structural/high-level** — show module structures, function signatures, key logic flows, and config. NOT full line-by-line production code, NOT pseudo code. The candidate should understand the current system by browsing the code.
...
14. **NEVER prescribe technology choices** — Do NOT tell the candidate what tech stack, framework, infrastructure, database, message broker, HTTP framework (Express / Fastify / NestJS / Koa / etc.), ORM, or any tool to use anywhere in ANY file. Only describe/show what currently exists. The candidate's technology choices and justifications are a core evaluation criterion. This applies to README.md, constraints, code files, comments — NOWHERE should the task mandate what the candidate must use.
15. **Code files must have realistic folder structure** — follow standard Node.js project conventions (e.g., `src/` with `routes/`, `controllers/`, `services/`, `models/`, `repositories/`, `middleware/`, `workers/`, `config/`). The folder structure itself communicates the current architecture.
