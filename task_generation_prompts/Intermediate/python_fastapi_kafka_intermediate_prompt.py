...
especially focusing on how Kafka is used in FastAPI-based event-driven systems at an intermediate level — such as for reliable asynchronous service communication, event sourcing patterns, consumer group orchestration, dead-letter topic strategies, and production-grade async message processing?
"""
...
## GOAL
As a technical architect experienced in event-driven architectures, Python FastAPI, and Apache Kafka, you are given a list of real-world scenarios and proficiency levels for FastAPI + Kafka development.
Your job is to generate an entire task definition, including code files, README.md, expected outcomes etc. that can be effectively used to assess the candidate's ability to design and implement production-grade event-driven FastAPI services that communicate via Kafka at an intermediate level.

## INSTRUCTIONS

### Nature of the Task
- Task must ask to implement a feature from scratch, refactor existing code, or fix complex bugs involving Kafka producer/consumer patterns, event-driven workflows, or distributed message processing within FastAPI applications.
...
- The complexity of the task must align with INTERMEDIATE proficiency level (3-5 years experience in FastAPI and Kafka).
...
- **CRITICAL SCOPE**: The task must have 3-4 focused objectives that test intermediate Kafka concepts:
  ...
  - Backpressure handling and batch processing patterns with aiokafka
  - Structured logging and observability for Kafka message flows
  - Data consistency patterns between Kafka events and database state (outbox pattern)
  - FastAPI lifespan management for producer/consumer lifecycle
- For INTERMEDIATE level, the scenarios should require architectural thinking and involve:
  ...
  - Graceful shutdown with in-flight message handling via FastAPI lifespan
- The task should involve 2-3 Python services with Kafka as the message broker. At least one service should have significant architectural issues or missing functionality that the candidate must design and implement.
...
- Tests practical Kafka architecture and implementation skills in Python/FastAPI
...
- Focus on 2-3 Python services communicating via Kafka topics with intermediate-level patterns
- The task environment must use Docker Compose to run Kafka broker(s), any databases needed, and all Python services
...
- Use `python:3.11-slim` as the base Docker image for Python services.
...
### docker-compose.yml
- Must include Kafka broker, all Python service definitions, and any database services.
- Hardcoded configuration — no `.env` file references.
- Each service must expose its API port to the host.
...
- Kafka must be fully ready before Python services start.
...
{{
  ...
  "title": "Human-readable task title in '<action verb> <subject>' format, 50-80 characters. Examples: 'Architect Async Event Processing Pipeline with FastAPI and Kafka', 'Implement Idempotent Consumer Strategy for Order Events'.",
  "question": "Short description of the scenario and specific ask — what architectural issues exist and what the candidate must design/implement involving Kafka patterns in FastAPI",
  "code_files": {{
    "README.md": "Candidate-facing README following structure below",
    ".gitignore": "Comprehensive exclusions for Python, Kafka, Docker",
    "docker-compose.yml": "Docker Compose with Kafka broker, all Python services, and database(s)",
    "run.sh": "Deploy script — follows run.sh rules above",
    "kill.sh": "Cleanup script — follows kill.sh rules above",
    "<service-name>/Dockerfile": "Dockerfile for each service using python:3.11-slim",
    "<service-name>/requirements.txt": "Dependencies including fastapi, uvicorn, aiokafka, pydantic, sqlalchemy, etc.",
    "<service-name>/app/...": "Source code for each service",
  ...
  "definitions": {{
    ...
    "aiokafka": "An async Python client for Apache Kafka, designed to work with asyncio and ideal for integration with FastAPI's async architecture"
  }}
}}

## Code file requirements:
- Generate realistic multi-service folder structure with separate directories per service
- Code should follow modern Python best practices (type hints, async/await, Pydantic models, structured error handling)
...
- Provide realistic dependencies in requirements.txt that intermediate developers should be familiar with

## .gitignore INSTRUCTIONS:
Create a comprehensive gitignore file that covers all standard exclusions for Python: __pycache__/, *.py[cod], *$py.class, *.so, .Python, virtual environments (venv/, env/, .venv/), IDE configurations (.idea/, .vscode/, *.swp, *.swo), testing (.pytest_cache/, .coverage, htmlcov/), environment files (.env), log files (*.log, logs/), distribution (dist/, build/, *.egg-info/), Docker volumes, and other common development artifacts.
...
NEVER generate empty content - always provide substantial business context that explains what the candidate is working on and why proper Kafka integration with FastAPI is crucial.

### Service & Database Access

Provide all the access details the candidate needs to interact with the running environment:
  - **API endpoints for each service**: List each service name, its host as `<DROPLET_IP>`, and its exposed port (e.g., "Order Service: http://<DROPLET_IP>:8001", "FastAPI docs: http://<DROPLET_IP>:8001/docs")
...
### Objectives
  - Clear, measurable goals for the candidate appropriate for intermediate Kafka + FastAPI level
  ...
  - Include expectations for async message flow design, resilience, error handling, and observability
  ...
  - Examples of proper framing:
    ...
    * "Design async message flows that maintain data consistency between Kafka events and database state"
...
Provide practical guidance without revealing specific implementations:
  - Suggest exploring how async event-driven services handle failures differently from synchronous request-response
  ...
  - Suggest thinking about how to maintain consistency between database writes and Kafka publishes in an async context
...
### NOT TO INCLUDE in README: Make sure you do not include the following in the README.md file:
  - SETUP INSTRUCTIONS OR COMMANDS (docker-compose up, uvicorn, pip install, etc.)
  - Direct solutions or architectural decisions
  - Step-by-step implementation guides
  - Specific framework APIs or library methods that reveal the solution (e.g., specific aiokafka methods, FastAPI lifespan patterns)
...
2. **code_files** must include README.md, .gitignore, docker-compose.yml, run.sh, kill.sh, Dockerfiles, requirements.txt, and source files for each service
