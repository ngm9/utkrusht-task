...
- **Async & Concurrency**: async/await patterns, event loop considerations, blocking vs non-blocking I/O, task management, thread pool / process pool strategies, GIL implications on concurrency strategy
- **Data Access Patterns**: Database access strategies, connection management/pooling, caching layers, read replicas, N+1 prevention with eager loading, migration strategies
- **Background Task Processing**: Task queue design, scheduled jobs, long-running task management, result backends, retry/dead-letter strategies
...
- **API Design & Gateway Patterns**: RESTful API design, request/response model design, API versioning, rate limiting, middleware architecture, authentication/authorization
- **Caching & Performance**: Caching strategies, cache invalidation patterns, response caching, query caching, serialization performance
- **Transaction Management**: Distributed transactions, saga pattern, eventual consistency, compensating transactions, idempotency
- **Observability Design**: Structured logging, distributed tracing, metrics, health checks, alerting
- **Deployment & Scaling**: Worker process configuration, horizontal scaling, stateless service design, container sizing, graceful shutdown, deployment strategies
...
{{
   ...
   "code_files": {{
      ...
      "// Build/config files": "pyproject.toml or requirements.txt (with dependencies), settings/config module (e.g., src/app/core/config.py), .env.example, docker-compose.yml if applicable",
      "// Source code files": "FastAPI app entrypoint (e.g., src/app/main.py), router/endpoint modules, service modules, Pydantic schema modules, ORM model modules, repository/DAO modules, dependency providers, message consumer/producer modules, background worker modules — all in proper package structure (e.g., src/app/api/..., src/app/services/..., src/app/models/...)",
      "// SQL/schema files": "Database schema or migration files if applicable (e.g., alembic/versions/*.py, schema.sql)"
   }},
...
The repo must contain the actual Python project folder structure with real code files showing the current system. This is NOT pseudo code and NOT a full line-by-line production implementation. It is structural/high-level code that shows:

**What the code files SHOULD contain:**
- Real module/file structure with proper package organization (e.g., `src/app/services/order_service.py`, `src/app/api/v1/routes/orders.py`)
- Function/method signatures with brief high-level logic (what the function does, not every line)
- Pydantic schema and ORM model class definitions with fields and relationships
- Configuration files (pyproject.toml or requirements.txt with dependencies, config/settings module, .env.example)
- Router/endpoint modules showing route definitions and what they handle
- Service layer modules showing business logic flow at a high level
- Repository/DAO modules or classes
- Message consumer/producer modules if applicable
- Docker-compose.yml if the system uses containers
- Alembic migration files or schema.sql if applicable
...
**Style guideline:**
- Each code file should feel like reading a real project's source — proper imports, decorators, type hints, class structure
...
- Follow standard Python project conventions (e.g., `src/app/...` layout with `api/`, `services/`, `models/`, `schemas/`, `repositories/`, `workers/`, `core/`)
- Include build files, config files, source files, and schema/migration files as appropriate
...
2. **This is a DESIGN task with real code context** — The repo contains actual Python project code files (source modules, config, build files) showing the current system. The candidate reads these to understand the current state, then writes their design in DESIGN.md.
...
11. **Code files must be structural/high-level** — show class structures, function signatures, key logic flows, and config. NOT full line-by-line production code, NOT pseudo code. The candidate should understand the current system by browsing the code.
...
15. **Code files must have realistic folder structure** — follow standard Python project conventions (e.g., `src/app/...` with `api/`, `services/`, `models/`, `schemas/`, `repositories/`, `workers/`). The folder structure itself communicates the current architecture.
