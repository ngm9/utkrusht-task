...
Now that you understand the company context and role requirements, let me provide you with the specific inputs for generating a Java Spring Webservices assessment task.
...
2. What will the task look like? (Describe the type of implementation or fix required, the expected deliverables, and how it aligns with BASIC Java Spring Webservices proficiency)

Please provide a brief summary of your understanding before proceeding with the full task generation.
"""
PROMPT_JAVA_SPRING_WEBSERVICES_BASIC = """
## GOAL
As a senior Java architect super experienced in Java frameworks (Spring Boot, Spring MVC, Spring Web Services, Spring Data, Jackson, JAXB, Maven/Gradle), you are given a list of real world scenarios and proficiency levels for Java web services development.
...
- For BASIC level of proficiency, the questions must be more specific and less open ended. The scenarios must also be easily digestible and focus on fundamental Java Spring Webservices concepts like:
  - Building RESTful APIs with @RestController, @RequestMapping, @GetMapping, @PostMapping, @PutMapping, @DeleteMapping
  - Request/response binding with @RequestBody, @PathVariable, @RequestParam, @Valid
  - JSON serialization/deserialization with Jackson (@JsonProperty, @JsonIgnore, custom serializers)
  - Content negotiation for JSON and XML responses
  - Global exception handling with @ControllerAdvice and @ExceptionHandler, structured error responses
  - Input validation with JSR-303/380 bean validation (@NotNull, @NotBlank, @Min, @Max, @Pattern, @Valid)
  - Consuming external REST APIs with RestTemplate or WebClient (basic synchronous usage)
  - Handling timeouts, retries, and error responses from downstream services
  - Introductory SOAP web services: @Endpoint, @PayloadRoot, WSDL, JAXB marshalling
  - Service layer patterns, DTO mapping, and controller-service-repository separation
  - Basic API security (HTTP Basic, API keys, simple JWT validation)
  - API documentation with SpringDoc/OpenAPI/Swagger
  - Testing with MockMvc, TestRestTemplate, WireMock for external service stubbing
...
- Tests practical Java Spring Webservices skills that require more than a simple AI query to solve, focusing on REST API design, consumption, validation, and error handling
- Time constraints: Each task should be finished within {{minutes_range}} minutes.
- At every time pick different real-world scenario from the list provided above to ensure variety in task generation.
- Focus on building or fixing REST/SOAP web services within a Spring Boot application
- Emphasize proper API design: correct HTTP methods, status codes, request/response contracts, validation, and error handling
...
- Include at least one controller, one service, and appropriate model/entity/DTO classes in the starter code
- If the task involves consuming an external API, provide a WireMock stub or mock configuration so the task is self-contained

## REQUIRED OUTPUT JSON STRUCTURE

{{
  "name": "task-name-in-kebab-case",
  "title": "Human-readable task title in '<action verb> <subject>' format, 50-80 characters. Describes what the candidate will do in plain English. Examples: 'Implement Transaction Lookup API with Proper Error Handling', 'Fix Order Service Validation and Downstream Timeout Handling', 'Build Shipment Tracking REST Endpoint with Input Validation'. The title should clearly convey the action (implement, fix, build, refactor, optimize, debug) and the subject (what system/feature/component). This is used for display purposes — 'name' is the kebab-case GitHub repo name, 'title' is the readable display name.",
  ...
  "code_files": {{
    ...
    "src/main/java/com/example/controller/ExampleController.java": "REST controller with request mappings",
    "src/main/java/com/example/service/ExampleService.java": "Service layer with business logic",
    "src/main/java/com/example/model/ExampleModel.java": "Model, entity, or DTO class",
    "additional_file.java": "Other Java source files as needed (repository, DTO, exception classes, config)"
  }},
  ...
  "pre_requisites": "Bullet-point list of tools, libraries, environment setup, and knowledge required. Include Java 11+, Maven 3.6+/Gradle 7+, IDE, Git, and Spring Webservices fundamentals (REST controllers, request/response binding, Jackson serialization, validation, exception handling, RestTemplate/WebClient, basic SOAP awareness, MockMvc/WireMock testing).",
  "answer": "High-level solution approach describing main components and flow.",
  "hints": "Single line suggesting focus area. Example: 'Focus on proper HTTP status codes, request validation with @Valid, and structured error responses via @ControllerAdvice'",
  "definitions": {{
    "REST": "Representational State Transfer — architectural style for designing networked applications using HTTP methods",
    "RestController": "Spring annotation combining @Controller and @ResponseBody for building REST APIs",
    "ControllerAdvice": "Global exception handler component for centralized error handling across controllers",
    "RestTemplate": "Synchronous HTTP client provided by Spring for consuming external REST APIs",
    "WebClient": "Non-blocking, reactive HTTP client from Spring WebFlux, also usable in synchronous mode",
    "MockMvc": "Spring test utility for testing MVC controllers without starting a full HTTP server",
    "WireMock": "Library for stubbing and mocking external HTTP services in integration tests"
  }}
}}

## Code file requirements:
- More than 1 files can be generated but make sure they are included in the JSON structure correctly.
- Code should follow modern Java best practices and Spring Boot conventions
- Use proper package structure (com.example.taskname.controller, service, repository, model, dto, exception, config, etc.)
- Use appropriate Spring annotations (@RestController, @Service, @Repository, @RequestBody, @PathVariable, @Valid, @ControllerAdvice, @ExceptionHandler, etc.)
...
## README.md STRUCTURE (Java / Spring Webservices)
...
Define goals focusing on outcomes for a BASIC-level Java Spring Webservices task:
...
Verification approaches for the task:
  ...
  - Focus on observable outcomes (API responses, status codes, error payloads, external service handling)
...
2. **name** must be short, descriptive, kebab-case (e.g., "transaction-api-error-handling")
...
8. **hints** must be a single line; **definitions** must include relevant Java/Spring Webservices terms
...
12. **Emphasize web services patterns** — proper REST design, HTTP semantics, validation, error handling, and API contracts are key
