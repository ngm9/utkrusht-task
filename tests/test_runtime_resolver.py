    ...
    *,
    ...
    def table(name: str):
        ...
        if name == "task_template_match":
            ...
            select.execute.return_value.data = rows  # _load_active_templates
...
def test_is_match_fresh_returns_true_when_model_and_version_match():
    ...
    assert _is_match_fresh(_CLASSIFIER_MODEL, 3, t) is True
...
def test_is_match_fresh_stale_when_model_differs():
    ...
    assert _is_match_fresh("older-model", 1, t) is False
...
def test_is_match_fresh_stale_when_registry_version_bumped():
    ...
    assert _is_match_fresh(_CLASSIFIER_MODEL, 4, t) is False
...
def test_is_match_fresh_no_match_rows_survive_version_bumps():
    ...
    assert _is_match_fresh(_CLASSIFIER_MODEL, 1, None) is True
    assert _is_match_fresh("older-model", 1, None) is False


# ── _row_to_template ─────────────────────────────────────────────────


def test_row_to_template_round_trip():
    row = _row(template_id="utkrusht-python", registry_version=7)
    spec = _row_to_template(row)
...
def test_row_to_template_defaults_missing_columns():
    spec = _row_to_template({
        "template_id": "minimal",
        "primary_runtime": "python",
...
def test_cache_hit_returns_cached_match_no_llm():
    ...
    with patch("infra.classifier.llm_classifier.classify_match") as llm:
        plan = resolve_plan(_comps(("Python", "INTERMEDIATE")), supabase=client)
...
def test_cache_hit_stale_model_triggers_reclassify():
    ...
    assert upserts[0]["classifier_model"] == _CLASSIFIER_MODEL
...
def test_cache_hit_stale_registry_version_triggers_reclassify():
    ...
    with patch(
        ...
        resolve_plan(_comps(("Python", "INTERMEDIATE")), supabase=client)
...
def test_cache_miss_calls_llm_and_writes_row():
    """Empty cache → classify_match → upsert into task_template_match."""
    template_row = _row()
    upserts: list = []
    client = _make_supabase(
        match_row=None,
...
def test_no_cache_falls_through_to_llm():
    ...
    with patch(
        ...
        plan = resolve_plan(_comps(("Python", "INTERMEDIATE")))

    llm.assert_called_once()
    # With no DB, active templates list passed to LLM is empty — see how
    # classify_match would handle that in real life (every output =
    # no_match). For this test we only verify resolve_plan doesn't crash.
...
def test_cache_lookup_failure_falls_through_to_llm():
    """Supabase lookup raises mid-call → resolve_plan still produces a plan."""
    client = MagicMock()
    ...
    # Match lookup will raise; but resolve_plan also tries to load
