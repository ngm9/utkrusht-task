...
| `question` leaks a file name/path, directory path, or function/method reference | Generator named the specific file or function the defect lives in — hands the candidate the answer's location instead of letting them diagnose it | Rewrite the sentence around it in scenario/behavior terms (what the system does wrong, not which file does it wrong); the fact stays, the code reference goes |
| `question` states the answer directly ("the bug is in...", "you should change...", "change line N") | Generator drifted from ask into solution — a real task-quality defect, not just formatting | Rewrite the closing ask as an imperative goal ("fix X so that Y behaves correctly") with no mention of the mechanism or fix |
