...
from infra.utils import format_outcomes
...
def _run_quality_autofix_on_non_tech_blob(task_data_for_db: dict) -> None:
    """Run the content-quality judge on the assembled non-tech task and
    splice any rewrites in place. Mutates ``task_data_for_db``.

    Non-tech uses ``task_blob.task_overview`` (a list of bullets) where the
    coding flow uses ``task_blob.short_overview`` — we map at the call
    boundary so the quality eval prompt does not have to know the
    flow-specific name. ``pre_requisites`` is already top-level.
    """
    ...
    flat = {
        ...
        # Map task_overview ↔ short_overview at the eval boundary
        "short_overview": task_blob.get("task_overview", []),
    ...
    if report.rewrites_applied:
        logger.info(
            f"Non-tech quality eval autofixed "
    ...
    for field in ("title", "question", "outcomes"):
        if field in report.rewrites_applied:
            task_blob[field] = patched[field]
    # Map the LLM's short_overview rewrite back to task_overview.
    if "short_overview" in report.rewrites_applied:
        task_blob["task_overview"] = patched["short_overview"]
...
def create_test_task(competency_file: Path, background_file: Path, scenarios_file: Path = None, env: str = "dev") -> Dict:
    ...
    try:
        ...
        for i, competency in enumerate(competencies):
            ...
            # Outcomes as List[str] (spec 003 — same shape as the coding flow).
            outcomes_value = format_outcomes(task_data.get("outcomes", ""))
            ...
            # Convert all empty strings to None — keep list fields list-shaped
            # by stashing them, running the helper, and putting them back.
            _list_fields = {
                "pre_requisites": task_data_for_db["pre_requisites"],
                "outcomes": task_data_for_db["task_blob"]["outcomes"],
                "task_overview": task_data_for_db["task_blob"]["task_overview"],
            }
            task_data_for_db = convert_empty_to_none(task_data_for_db)
            task_data_for_db["pre_requisites"] = _list_fields["pre_requisites"]
            task_data_for_db["task_blob"]["outcomes"] = _list_fields["outcomes"]
            task_data_for_db["task_blob"]["task_overview"] = _list_fields["task_overview"]

            # Content-quality eval (spec 003) — same layer as the coding flow.
            try:
                _run_quality_autofix_on_non_tech_blob(task_data_for_db)
            except Exception as e:
